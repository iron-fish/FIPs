...
With the upcoming switch to FishHash, the network difficulty will drop dramatically. This opens up some potential re-organization attacks on the chain. Although these attacks would still be expensive to execute, it would be best to further mitigate them by 'locking in' the hardfork. This is where a hash-based checkpoint would come in. After the hardfork, the network would be able to add a hash-based checkpoint to the consensus layer. This would prevent any re-organization attacks that would remove the hardforked block from the chain.

## Specification

We propose adding a new parameter to the consensus layer, `checkpoints`. This would be a mapping of sequence -> block hash. If a node adds the designated block at that sequence, it will never re-organize its chain prior to that block. Other blocks at that sequence that do not match the hash checkpoint can still be gossiped and added to the chain, but they will never be connected to the main chain if the checkpoint has already been met. Below are a few example scenarios to illustrate how this would work:
